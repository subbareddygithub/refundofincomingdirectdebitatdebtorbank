...
@Service
public class Financial GatewayRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Financial GatewayRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.financial gateway.hosturl}")
	private String url;
	
	@Value("${spring.api.financial gateway.context-path}")
	private String contextPath;

	@Value("${spring.api.financial gateway.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Financial GatewayRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Financial GatewayRestProcessor.class.getName());
