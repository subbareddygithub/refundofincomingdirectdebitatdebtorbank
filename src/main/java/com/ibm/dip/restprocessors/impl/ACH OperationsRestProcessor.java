...
@Service
public class ACH OperationsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(ACH OperationsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.ach operations.hosturl}")
	private String url;
	
	@Value("${spring.api.ach operations.context-path}")
	private String contextPath;

	@Value("${spring.api.ach operations.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  ACH OperationsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ ACH OperationsRestProcessor.class.getName());
