...
@Service
public class Payment InitationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Payment InitationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.payment initation.hosturl}")
	private String url;
	
	@Value("${spring.api.payment initation.context-path}")
	private String contextPath;

	@Value("${spring.api.payment initation.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Payment InitationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Payment InitationRestProcessor.class.getName());
