...
@Service
public class Internal Bank AccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Internal Bank AccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.internal bank account.hosturl}")
	private String url;
	
	@Value("${spring.api.internal bank account.context-path}")
	private String contextPath;

	@Value("${spring.api.internal bank account.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Internal Bank AccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Internal Bank AccountRestProcessor.class.getName());
