...
@Service
public class Correspondent Bank OperationsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Correspondent Bank OperationsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.correspondent bank operations.hosturl}")
	private String url;
	
	@Value("${spring.api.correspondent bank operations.context-path}")
	private String contextPath;

	@Value("${spring.api.correspondent bank operations.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Correspondent Bank OperationsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Correspondent Bank OperationsRestProcessor.class.getName());
